...
class TrendEfficiencyMomentumError(ValueError):
    pass


@dataclass(frozen=True, slots=True)
class TrendEfficiencyMomentumSpec:
    lookback_hours: int = 336
    trend_ema_hours: int = 400
    efficiency_hours: int = 168
    efficiency_threshold: float = 0.25

    def __post_init__(self) -> None:
        if min(self.lookback_hours, self.trend_ema_hours, self.efficiency_hours) <= 0:
            raise TrendEfficiencyMomentumError("strategy hour windows must be positive")
        if not 0.0 < self.efficiency_threshold <= 1.0:
            raise TrendEfficiencyMomentumError("efficiency threshold must be in (0, 1]")


@dataclass(frozen=True, slots=True)
class TrendEfficiencyMomentumStrategy:
    spec: TrendEfficiencyMomentumSpec
    timeframe_hours: int
    name: str = "trend_efficiency_gated_momentum"

    def __post_init__(self) -> None:
        if self.timeframe_hours <= 0:
            raise TrendEfficiencyMomentumError("timeframe hours must be positive")
        for hours in (
            self.spec.lookback_hours,
            self.spec.trend_ema_hours,
            self.spec.efficiency_hours,
        ):
            if hours % self.timeframe_hours != 0:
                raise TrendEfficiencyMomentumError("strategy windows must divide by timeframe")
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        one_bar_move = (logged - logged.shift(1)).abs()
        efficiency_bars = self._bars(self.spec.efficiency_hours)
        net_move = (logged - logged.shift(efficiency_bars)).abs()
        path_length = one_bar_move.rolling(
            efficiency_bars,
            min_periods=efficiency_bars,
        ).sum()
        efficiency = net_move / path_length.mask(path_length <= 0.0)
        ...
        gate = efficiency >= self.spec.efficiency_threshold
...
@dataclass(frozen=True, slots=True)
class TrendEfficiencyEvaluation:
    dataset: str
    period: str
    total_return: float
    sharpe_ratio: float
...
def _evaluate(
...
) -> TrendEfficiencyEvaluation | None:
    data = _slice_period(dataset.data, start, end)
    if len(data) < 2:
        return None
    strategy = TrendEfficiencyMomentumStrategy(spec, dataset.timeframe_hours)
    ...
    return TrendEfficiencyEvaluation(
...
def _frame(rows: list[TrendEfficiencyEvaluation]) -> pd.DataFrame:
...
def run_research(root: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    pre_rows: list[TrendEfficiencyEvaluation] = []
    ...
    stress_rows: list[TrendEfficiencyEvaluation] = []
    shadow_rows: list[TrendEfficiencyEvaluation] = []
    zero_cost_rows: list[TrendEfficiencyEvaluation] = []
    for dataset in datasets:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run trend-efficiency gated momentum study")
    ...
    pre_stress.to_csv(root / "trend_efficiency_momentum_pre_stress.csv", index=False)
    stress.to_csv(root / "trend_efficiency_momentum_stress_2026.csv", index=False)
    shadow.to_csv(root / "trend_efficiency_momentum_shadow.csv", index=False)
    zero_cost.to_csv(root / "trend_efficiency_momentum_zero_cost.csv", index=False)
    print("pre-stress")
